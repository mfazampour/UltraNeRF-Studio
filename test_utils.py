...
def visualize_poses(original_poses, perturbed_poses, sample_ratio=1.0, arrow_length=0.5, title=None):
    ...
    # Reorient axes
    original_reoriented = original_translations[:, [0, 2, 1]]  # Map X, Y, Z to width, slices, height
    ...
    # Plot translations
    ax.scatter(original_reoriented[:, 0], original_reoriented[:, 1], original_reoriented[:, 2], 
               color='blue', label='Original Translations', s=50, depthshade=True)
    ax.scatter(perturbed_reoriented[:, 0], perturbed_reoriented[:, 1], perturbed_reoriented[:, 2], 
               color='red', label='Perturbed Translations', s=50, depthshade=True)
    
    # Plot directions as quivers
    for i in range(num_samples):
        ax.quiver(
            original_reoriented[i, 0], original_reoriented[i, 1], original_reoriented[i, 2],
            original_directions_reoriented[i, 0], original_directions_reoriented[i, 1], original_directions_reoriented[i, 2],
            color='blue', length=arrow_length, normalize=True, alpha=0.7
        )
        ax.quiver(
            perturbed_reoriented[i, 0], perturbed_reoriented[i, 1], perturbed_reoriented[i, 2],
            perturbed_directions_reoriented[i, 0], perturbed_directions_reoriented[i, 1], perturbed_directions_reoriented[i, 2],
            color='red', length=arrow_length, normalize=True, alpha=0.7
        )
